...
class TestLibraryCustomCreateMessages(asynctest.TestCase):
    def setUp(self):
        ...
        patcher = patch('pkg.botgotsthis.library.custom.split_message',
                        autospec=True)
        self.addCleanup(patcher.stop)
        self.mock_split = patcher.start()

        patcher = patch('pkg.botgotsthis.library.custom.convert_field')
        self.addCleanup(patcher.stop)
        self.mock_convert = patcher.start()
        self.mock_convert.side_effect = lambda args: args.default

        patcher = patch('pkg.botgotsthis.library.custom.format', autospec=True)
        self.addCleanup(patcher.stop)
