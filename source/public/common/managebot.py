...
def botManageBot(db, sendMessage, nick, message, msgParts):
    params = db, sendMessage, nick, message, msgParts
    
    if msgParts[1].lower() in methods:
        return methods[msgParts[1].lower()](*params)
    return False

def manageListChats(db, sendMessage, nick, message, msgParts):
    channels = [c for c in globals.channels.keys()]
    sendMessage('Twitch Chats: ' + ', '.join(channels))
    return True

def manageAutoJoin(db, sendMessage, nick, message, msgParts):
    if len(msgParts) < 3:
        return False
...
def manageBanned(db, sendMessage, nick, message, msgParts):
    if len(msgParts) < 3:
        return False
